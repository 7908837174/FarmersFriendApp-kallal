...
let bagItems = [];  // Array to store the cart items
    ...
    items.forEach(item => {
        ...
        productElement.innerHTML = `
            ...
            <div class="product-details">
                ...
                <p><span class="current-price">$${item.current_price}</span> <span class="original-price">${item.original_price}</span></p>
                <p>Discount: ${item.discount_percentage}% OFF</p>
                <button class="add-to-cart" onclick="addToCart('${item.item_id}')">Add to Cart</button>
            </div>
        `;
...
a1.addEventListener("click", (e) => {
    e.preventDefault();
    window.location.href = "bag.html";  // Redirect to the bag page
});
